...
public class InMemoryFilmStorage implements FilmStorage {
    ...
    public Film updateFilm(Film updateFilm) {
        ...
        if (StringUtils.isBlank(updateFilm.getName())) {
            log.error("На обновление передано пустое название фильма.");
            throw new ValidationException("Название фильиа не может быть пустым.");
        }
        if (updateFilm.getDescription().length() > 200) {
            log.error("Описание фильма больше допустимого количества символов: {}", updateFilm.getDescription().length());
            throw new ValidationException("Описание фильиа должно содержать нее более 200 символов.");
        }
        if (updateFilm.getDuration() <= 0) {
            log.error("Передана некорректная продолжительность фильма: {}", updateFilm.getDuration());
            throw new ValidationException("Продолжительность фильиа должна быть положительной.");
        }
        if (updateFilm.getReleaseDate().isBefore(LocalDate.of(1895, 12, 28))) {
            log.error("Передана некорректная дата релиза: {}", updateFilm.getReleaseDate());
            throw new ValidationException("Дата релиза не может быть ранее 28.12.1895г.");
        }
